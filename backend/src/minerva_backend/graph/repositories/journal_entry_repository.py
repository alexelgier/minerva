...
class JournalEntryRepository(BaseRepository[JournalEntry]):
    ...
    async def find_by_date(self, entry_date: date) -> Optional[JournalEntry]:
        """
        ...
        async with self.connection.session_async() as session:
            result = await session.run(query, entry_date=entry_date)
        ...
        async with self.connection.session_async() as session:
            ...
            if record and record["total_entries"] > 0:
                ...
                return {
                    ...
                    "oldest_entry_date": (
                        date(oldest_date.year, oldest_date.month, oldest_date.day)
                        if oldest_date
                        else None
                    ),
                    "newest_entry_date": (
                        date(newest_date.year, newest_date.month, newest_date.day)
                        if newest_date
                        else None
                    ),
                }
