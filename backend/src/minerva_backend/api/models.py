...
class JournalSubmission(BaseModel):
    ...
    date: str = Field(
        ..., description="Journal date in YYYY-MM-DD format", examples=["2025-09-19"]
    )
    text: str = Field(
        ..., min_length=10, max_length=50000, description="Journal entry text content"
    )
...
class CurationAction(BaseModel):
    ...
    curated_data: Dict[str, Any] = Field(
        default_factory=dict, description="Curated data (required for accept action)"
    )

    @field_validator("curated_data")
    def validate_curated_data_for_accept(
        cls, curated_data: Dict[str, Any], info: ValidationInfo
    ):
...
class PendingPipelinesResponse(BaseResponse):
    ...
    pending_pipelines: List[Dict[str, Any]] = Field(
        default_factory=list, description="List of pending pipeline workflows"
    )
...
class PendingCurationResponse(BaseResponse):
    ...
    journal_entries: Dict[str, JournalEntryCuration] = Field(
        default_factory=dict, description="List of pending journals with curation tasks"
    )
    stats: CurationStats = Field(..., description="Curation statistics")
...
class ProcessingWindowsResponse(BaseResponse):
    ...
    default_window: Dict[str, Any] = Field(
        ..., description="Default processing window configuration"
    )
    ...
    manual_control: bool = Field(
        default=False, description="Whether manual control is active"
    )
...
class HealthCheckResponse(BaseResponse):
    ...
    curation_stats: CurationStatsResponse = Field(
        ..., description="Current curation statistics"
    )
